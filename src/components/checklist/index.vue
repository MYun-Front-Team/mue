<template>
  <div>
    ...
    <div class="weui-cells weui-cells_checkbox">
      <label class="weui-cell weui-check_label" :class="{'m-checklist-label-left': labelPosition === 'left'}" :for="`checkbox_${uuid}_${index}`" v-for="(one, index) in currentOptions">
        <div class="weui-cell__hd">
          <input type="checkbox" class="weui-check" :name="`m-checkbox-${uuid}`" :value="getKey(one)" v-model="currentValue" :id="`checkbox_${uuid}_${index}`" :disabled="ifDisable(getKey(one))">
          <i class="weui-icon-checked m-checklist-icon-checked"></i>
        </div>
...
.weui-cells_checkbox .weui-check:checked + .m-checklist-icon-checked:before {
