<template>
  <div class="m-slider">
    <div class="m-swiper" :style="{height: xheight}">
      <slot></slot>
      <div class="m-swiper-item" v-for="(item, index) in list" @click="clickListItem(item)" :data-index="index">
        <a href="javascript:">
          <div class="m-img" :style="{backgroundImage: buildBackgroundUrl(item.img)}"></div>
          <p class="m-swiper-desc" v-if="showDescMask">{{item.title}}</p>
        </a>
      </div>
    </div>
    <div :class="[dotsClass, 'm-indicator', 'm-indicator-' + dotsPosition]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i class="m-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
