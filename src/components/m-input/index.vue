<template>
	<div class="mue-m-input weui-cell" :class="{'weui-cell_warn': !valid}">
    ...
    <div class="weui-cell__ft">
      ...
      <icon class="m-input-icon" type="warn" :title="!valid ? firstError : ''" v-show="!novalidate && !equalWith && ((touched && !valid && firstError) || (forceShowError && !valid && firstError))"></icon>
      <icon class="m-input-icon" type="warn" v-if="!novalidate && hasLengthEqual && dirty && equalWith && !valid"></icon>
      <icon type="success" v-show="!novalidate && equalWith && equalWith === currentValue && valid"></icon>

      <icon type="success" class="m-input-icon" v-show="novalidate && iconType === 'success'"></icon>
      <icon type="warn" class="m-input-icon" v-show="novalidate && iconType === 'error'"></icon>
...
@import '../../styles/weui/widget/weui_cell/weui_form/weui_vcode.less';

.m-input-icon.m-input-icon {
  font-size: .28rem;
}
.m-input-icon.weui-icon-warn:before, .m-input-icon.weui-icon-success:before {
  font-size: .28rem;
}
.mue-m-input .weui-icon {

}
.mue-m-input.weui-cell_vcode {
  padding-top: 0;
  padding-right: 0;
  padding-bottom: 0;
}
